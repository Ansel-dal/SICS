...
namespace SICS.Client.Servicios.Implementacion
{
    public class PrestamoServicio : IPrestamoServicio
    {
        ...
        public async Task<ResponseDTO<List<PrestamoDTO>>> Buscar(string estadoPrestamo, string codigoConsumidor)
        {
            var result = await _http.GetFromJsonAsync<ResponseDTO<List<PrestamoDTO>>>($"api/prestamo/Buscar?estadoPrestamo={estadoPrestamo}&codigoConsumidor={codigoConsumidor}");
            return result!;
        }
